...
export class DanhSachGheComponent implements OnInit {
  ...
  chonGhe(objGhe) {
    if (objGhe.TrangThaiDangChon) {
      this.dsGheDangDat.push(objGhe.ghe);
      this.soGheDaDat++;
      this.tongTien += objGhe.ghe.Gia;
    } else {
      let index = this.dsGheDangDat.findIndex(item => {
        return item.SoGhe === objGhe.ghe.SoGhe;
      });
      this.dsGheDangDat.splice(index, 1);
      this.soGheDaDat--;
      this.tongTien -= objGhe.ghe.Gia;
    }
  }
  ...
  remove(index, ghe) {
    ...
    this.tagListItemGhe.map(item => {
      if (item.ghe.SoGhe === ghe.SoGhe) {
        console.log(item.ghe.SoGhe);
        item.TrangThaiDangChon = false;
      }
    })
  }
